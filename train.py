...
def train_generator(save_to_ram=False):
    ...
    while True:
        ...
        for start in range(0, len(ids_train_split), batch_size):
            x_batch = []
            y_batch = []
            dx = np.random.randint((cols-crop_size[1])/32 + 1)*32
            dy = np.random.randint((rows-crop_size[0])/32 + 1)*32
...
if __name__ == '__main__':
    ...
    rows, cols = 1024, 1536
    crop_size = (1024, 1536)
    learning_rate = [0.000250000011874, 2e-3]
    input_mean = 0.
    decay = 0.5
    offset = 881
    ...
    for idx in range(len(activations)):
        ...
        with open('nohup.out.' + name, 'w') as f:
            ...
            model = create_model(shape=(None, None, 3),
                                 num_blocks=5,
                                 kernel=3,
                                 filter=[4,8,16,32,32,32,32,32,16,8,4],
                                 encoding_dilation=1,
                                 decoding_dilation=1,
